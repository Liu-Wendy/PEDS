...
public class Parser {
    ...
    public ComposedAutomata generateComposedAutomaton(String cfgfile, int bound) {
    ...
    private Automata generateAutomatonVariables(BindInfo b, String prefix) {
        ...
        for (Transition tran : aprototype.transitions) {
            String label = tran.label;
            if (b.map.containsKey(label)) {
                label = b.map.get(label);
            } else label = prefix + label;

            if (ca.allLables.contains(label)) {
                ca.sharedlabels.add(label);
            } else {
                ca.allLables.add(label);
            }
        }
